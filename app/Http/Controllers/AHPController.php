...
class AHPController extends Controller
{
    ...
    public function index_perhitungan_kriteria(Request $request)
    {
        ...
        $matriksPerbandingan = DB::table('matriks_perbandingan_kriteria as mpk')
            ->join('sub_kriteria as sk', function ($join) use ($request) {
                $join->on('sk.kategori_id', '=', 'mpk.kategori_id')
                    ->where('sk.kriteria_id', '=', $request->kriteria_id);
            })
            ->where('mpk.kriteria_id', $request->kriteria_id)
            ->select('mpk.*', 'sk.id as sub_kriteria_id', 'sk.nama as nama_sub_kriteria', 'sk.kategori_id')
            ->orderBy('mpk.id', 'asc')
            ->get();

        $matriksNilai = DB::table('matriks_nilai_kriteria as mnk')
            ->join('sub_kriteria as sk', function ($join) use ($request) {
                $join->on('sk.kategori_id', '=', 'mnk.kategori_id')
                    ->where('sk.kriteria_id', '=', $request->kriteria_id);
            })
            ->where('mnk.kriteria_id', $request->kriteria_id)
            ->select('mnk.*', 'sk.id as sub_kriteria_id', 'sk.nama as nama_sub_kriteria', 'sk.kategori_id')
            ->get();
    ...
    public function ubah_matriks_perbandingan_kriteria(Request $request)
    {
        ...
        $matriksPerbandingan = DB::table('matriks_perbandingan_kriteria as mpk')
            ...
            ->join('kategori as kat', 'kat.id', '=', 'mpk.kategori_id_banding')
            ->where('mpk.kriteria_id', $request->kriteria_id)
            ->where('mpk.kategori_id', $request->kategori_id)
            ->select(
                'mpk.*',
                'sk.id as sub_kriteria_id',
                'sk.nama as nama_sub_kriteria',
                'sk.kategori_id',
                'kat.nama as nama_kategori_banding' 
            )
            ->get();

        $allSubKriteria = DB::table('sub_kriteria')
            ->where('kriteria_id', $request->kriteria_id)
            ->get()
            ->keyBy('kategori_id');

        foreach ($matriksPerbandingan as $value => $item) {
            if (isset($allSubKriteria[$item->kategori_id_banding])) {
                $matriksPerbandingan[$value]->nama_sub_kriteria_banding = $allSubKriteria[$item->kategori_id_banding]->nama;
            }
        }
